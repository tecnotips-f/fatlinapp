...
// ⚠️  IMPORTANTE: cambiar a false antes de lanzar a producción
// true  → SANDBOX    (pruebas — firma PayPal NO verificada)
// false → PRODUCCIÓN (pagos reales — firma PayPal verificada obligatoriamente)
...
exports.mpWebhook = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    // Separar por el ÚLTIMO guion bajo — soporta UIDs con guiones bajos
    const lastIdx = extRef.lastIndexOf("_");
    const uid     = extRef.substring(0, lastIdx);
    const level   = parseInt(extRef.substring(lastIdx + 1));
...
exports.ppCaptureOrder = functions
  ...
  try {
    ...
    if (capture.status === "COMPLETED" || completedCapture) {
      ...
      // ✅ FIX SANDBOX: ORDER_ALREADY_CAPTURED = el pago sí ocurrió — confirmar igual
      ...
      res.status(400).json({ error: "Captura no completada", status: capture.status, ppError, ppDesc });
    }
