  ...
  <div class="cert-wrapper">
    <div class="cert-diplomado-preview" id="preview-diplomado">
      <div class="cert-diplomado" id="cert-inner-diplomado">
        ...
        <!-- ══ Watermark ══ -->
        <div class="dc-wm">
          <svg width="367" height="367" viewBox="0 0 100 100">
            <path d="M50 90 L90 70 L50 50 L10 70 Z" fill="#c8920a"/>
            <path d="M50 82 L90 62 L50 42 L10 62 Z" fill="#d4a017"/>
            <path d="M50 74 L90 54 L50 34 L10 54 Z" fill="#e0b020"/>
            <path d="M50 66 L90 46 L50 26 L10 46 Z" fill="#ecc030"/>
        ...
        <div class="dc-inn">
          ...
          <div class="dc-hdr">
            ...
            <div class="dc-hdr-brand">
              ...
              <svg width="160" height="160" viewBox="0 0 100 100"
                ...
                <!-- Cuadrado Turquesa fondo izquierda -->
                <rect x="5"  y="5"  width="60" height="60" fill="#009b9b"/>
                <!-- Cuadrado Negro superpuesto derecha -->
                <rect x="35" y="35" width="60" height="60" fill="#000000"/>
              </svg>
          ...
          <div class="dc-sigwrap">
            <div class="dc-sigrow">
              ...
              <!-- Sello institucional -->
              <svg viewBox="0 0 160 160" width="127" height="127">
          ...
          <div class="dc-huella-block">
            <div class="dc-dvthin" style="width:100%;margin-bottom:7px;"></div>
          ...
          <div class="dc-foot" style="display:flex;align-items:center;justify-content:space-between;width:100%;padding:12px 16px;gap:11px;border-top:1.5px solid rgba(200,146,10,0.35);margin-top:0;background:transparent;">
            ...
            <div style="display:flex;flex-direction:column;align-items:center;gap:6px;flex-shrink:0;text-align:center;">
              ...
              <div id="${uid}-foot-hash" style="font-family:'Courier New',monospace;font-size:5px;color:#94a3b8;word-break:break-all;max-width:167px;">sha256:…</div>
            </div>

            <!-- QR verificación — extremo derecho, DENTRO del canvas -->
            <div style="display:flex;flex-direction:column;align-items:center;gap:8px;flex-shrink:0;">
              <div style="width:73px;height:73px;border:1px solid #c8920a;border-radius:3px;
                          padding:8px;background:#fff;box-shadow:0 2px 12px rgba(200,146,10,0.2);">
                <a href="${verifyUrl}" target="_blank" rel="noopener"
                   style="display:block;width:67px;height:67px;text-decoration:none;"
                   onclick="event.stopPropagation();window.open('${verifyUrl}','_blank');return false;">
                  <img src="${qrApiUrl}"
                       alt="QR verificación"
                       style="display:block;width:67px;height:67px;border-radius:1px;pointer-events:none;"
...
function initCertScale() {
  ...
  _watchScale('preview-diplomado', 1400, 990);
}
...
function printCert(tipo) {
  ...
  /* Móvil / tablet → siempre PNG (Web Share API o descarga directa) */
  /* Desktop → ventana de impresión (permite PDF nativo del navegador) */
...
async function _certDownloadPNG(certEl, tipo) {
  ...
  try {
    /* Expandir al tamaño nativo para captura */
    ...
    if (previewEl) { previewEl.style.overflow = 'visible'; previewEl.style.height = 'auto'; }

    /* Doble RAF → garantiza reflow completo antes de medir */
    await new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r)));

    /* Esperar imágenes (QR incluido) con timeout de seguridad */
    ...
    await Promise.race([
      ...
      new Promise(r => setTimeout(r, 3000))  /* máx 3s de espera */
    ]);
    await new Promise(r => setTimeout(r, 300));

    const captureScale = tipo === 'diplomado' ? 1 : 1.5;

    const canvas = await html2canvas(certEl, {
      scale:           captureScale,
      useCORS:         true,
      allowTaint:      true,   /* permite QR externo aunque no tenga CORS perfecto */
      backgroundColor: '#ffffff',
      logging:         false,
      imageTimeout:    10000,
      onclone: (doc) => {
        /* En el clon: quitar transforms para que html2canvas mida correctamente */
        const cl = doc.getElementById(certEl.id);
        if (cl) { cl.style.transform = 'none'; cl.style.position = 'relative'; }
      }
    });

    /* ── Intentar Web Share API (Android Chrome, iOS Safari 15+) ── */
    const blob = await new Promise(res => canvas.toBlob(res, 'image/png', 0.95));
    const file = new File([blob], fname, { type: 'image/png' });

    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      await navigator.share({ files: [file], title: 'Certificado Fatlin AI' });
      typeof window.showToast === 'function'
        && window.showToast('✅ Imagen lista para compartir o guardar', '#16a34a', 4000);
    } else {
      /* Fallback universal: createObjectURL — funciona en todos los navegadores */
      ...
      typeof window.showToast === 'function'
        && window.showToast('✅ Imagen descargada correctamente', '#16a34a', 4000);
    }

  } catch(err) {
    console.error('[_certDownloadPNG]', err);
    _certFallbackShare(certEl, fname, btn, origTxt);
  } finally {
...
/* ── Fallback: abre el cert en nueva pestaña para que el usuario lo guarde ── */
function _certFallbackShare(certEl, fname, btn, origTxt) {
  /* Renderizar el cert en un blob HTML que el usuario puede guardar/imprimir */
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8">
    ...
    </style></head><body>${certEl.outerHTML}
  ...
  if (btn) { btn.disabled = false; btn.textContent = origTxt || 'Descargar Diploma'; }
}

/* ── Ventana de impresión desktop ── */
function _certPrintWindow(certEl, tipo) {
  const nativeW = tipo === 'diplomado' ? 1400 : 1024;
  const nativeH = tipo === 'diplomado' ? 990  : 768;
  ...
  const bodyW  = isD ? '1400px' : '100%';
  const bodyH  = isD ? '990px' : 'auto';
