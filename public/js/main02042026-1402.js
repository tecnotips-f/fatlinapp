...
const db = getFirestore(app, "fatlindb");
// Exponer globalmente para payment.js y otros scripts no-módulo
window.db   = db;
window.auth = auth;
window.app  = app;
try { enableIndexedDbPersistence(db); } catch(e) { /* multi-tab o ya habilitado */ }

// Manejar resultado del redirect en móvil
getRedirectResult(auth).then(result => {
  if(result && result.user){
    document.getElementById('auth-screen').style.display = 'none';
    console.log('[Redirect Auth] OK:', result.user.email);
  }
}).catch(e => {
  if(e && e.code && e.code !== 'auth/null-user' && e.code !== 'auth/no-current-user'){
    console.error('[Redirect Auth Error]', e.code, e.message);
    const el = document.getElementById('error-msg');
    if(el){
      el.textContent = 'Error Google: ' + (e.code || e.message);
...
// Offset Ecuador UTC-5: sumamos 5h al instante UTC para que
// el "lunes 00:00 Ecuador" sea el punto de corte correcto
const ECU_OFFSET_MS = 5 * 60 * 60 * 1000; // UTC-5

function getWeekStart(offsetMs=0){
  // Trabajamos en "hora Ecuador virtual" sumando el offset
  const n=new Date(Date.now() + ECU_OFFSET_MS + (offsetMs||0));
  const day=n.getUTCDay(); // 0=Dom,1=Lun,...,6=Sab (en hora Ecuador)
  const diffToMon=day===0?-6:1-day;
  const mon=new Date(n);
  mon.setUTCDate(n.getUTCDate()+diffToMon);
  mon.setUTCHours(0,0,0,0);
  // La clave weekStart es la fecha en hora Ecuador (lunes local)
...
async function checkLigaPromocion(){
  ...
  try{
    ...
    try{
      ...
      snap.forEach(d=>{
        const dd=d.data();
        if(dd.weekStart===prevWs && (dd.ligaLevel||1)===(game.ligaLevel||1)) pl.push({uid:dd.uid,stars:dd.stars||0});
      });
...
async function saveProgress(){if(!game.user)return;const payload={
  ...
  };localStorage.setItem(`fatlin_save_${game.user.uid}`,JSON.stringify(payload));try{await setDoc(doc(db,`artifacts/${APP_ID}/users/${game.user.uid}/progress/current`),payload,{merge:true});const now=Date.now(),ws=getWeekStart();await setDoc(doc(db,`fatlin_ranking_${APP_ID}`,game.user.uid),{displayName:payload.displayName,level:game.level,stars:game.stars,avatar:selectedAvatar,updatedAt:now},{merge:true});await setDoc(doc(db,`fatlin_weekly_${APP_ID}`,`${ws}_${game.user.uid}`),{displayName:payload.displayName,level:game.level,stars:game.stars,avatar:selectedAvatar,weekStart:ws,uid:game.user.uid,ligaLevel:game.ligaLevel||1,updatedAt:now},{merge:true});}catch(e){}
...
function renderMap(){const nc=document.getElementById('nodes-container'),sv=document.getElementById('path-svg');nc.innerHTML='';sv.innerHTML='';const W=document.getElementById('map-scroll').offsetWidth;let d='';const visibleMax = game.level + 5; for(let i=1;i<=visibleMax;i++){const y=200+(i-1)*SPACING,x=(W/2)+Math.sin(i*.7)*(W*.25);d+=(i===1?'M':'L')+` ${x} ${y}`;const n=document.createElement('div');if(i<game.level){n.className='level-node completed';n.innerHTML=`<span class="lnlabel">${i}</span><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="3"><polyline points="20,6 9,17 4,12"/></svg>`;if(i%10===0)n.innerHTML+=`<span style="position:absolute;top:-10px;right:-8px;font-size:13px">🏅</span>`;}else if(i===game.level){
  const CERT_LEVELS=[81,161,241,321,401,481];
  const needsPay = CERT_LEVELS.includes(i) && game.paidLevels && !game.paidLevels.has(i-1);
  if(needsPay){
...
function getWeekCountdown(){
  // Calculamos cuánto falta para el próximo lunes 00:00 hora Ecuador
  const nowEcu=new Date(Date.now()+ECU_OFFSET_MS);
  const day=nowEcu.getUTCDay();
  const daysToMon=day===0?1:8-day; // días hasta el próximo lunes
...
function getWeekLabel(){
  // Fechas mostradas en hora Ecuador
  const ws=new Date(getWeekStart()+'T00:00:00Z');
  const o={day:'numeric',month:'long',timeZone:'America/Guayaquil'};
  const sun=new Date(ws.getTime()+6*86400000);
  return`${ws.toLocaleDateString('es-ES',o)} – ${sun.toLocaleDateString('es-ES',o)}`;
}
function buildRankItem(p,idx,isMe,maxL,showL=false,promoCut=0){
  ...
  const lb=(showL&&idx<5)?'<span class="lbadge">⚡ Liga</span>':'';
  const item=document.createElement('div');
  // Clases above-line / below-line según posición respecto al corte
  let lineClass = '';
  if(promoCut > 0){
    lineClass = idx < promoCut ? ' above-line' : ' below-line';
  }
...
async function loadLigaData(){
  ...
  try{
    ...
    let pl=[];
    // Sin limit() — traer toda la colección y filtrar en cliente por weekStart + ligaLevel
    // Necesario porque Firestore no permite where+orderBy sin índice compuesto
    try{
      const s=await getDocs(collection(db,`fatlin_weekly_${APP_ID}`));
      s.forEach(d=>{
        const dat=d.data();
        if(dat.weekStart===ws && (dat.ligaLevel||1)===myLiga) pl.push({id:d.id,...dat});
      });
    }catch(e){
      console.warn('[Liga] Error leyendo weekly:',e);
    }
    pl.sort((a,b)=>(b.stars||0)-(a.stars||0)||(b.level||0)-(a.level||0));
    // Inyectar al jugador actual si aún no aparece (sin doc esta semana)
    if(game.user){
  ...
  pl.forEach((p,i)=>{
    ...
    fu.appendChild(buildRankItem({id:p.uid,displayName:p.displayName,level:p.level,stars:p.stars,avatar:p.avatar},i,isMe,ms,false,promoCut));
  });
  ...
  try{
    ...
    if([
      ...
      try{
        ...
        console.error('[Google Redirect Error]', e2.code, e2.message);
        showError('Error Google. Intenta con correo/contraseña.');
      }
    } else {
      console.error('[Google Auth Error]', e.code, e.message);
      showError('Error al iniciar sesión con Google: ' + (e.code || e.message));
    }
  }
};
...
async function generateDiploma(completedLevel) {
  ...
  if (typeof window.renderByLevel === 'function' && completedLevel >= 80) {
    ...
    const certId  = `FATLIN-${year}-${uidPart}-${completedLevel}`.toUpperCase();
    ...
    // ✅ 2. Primero definimos la URL de verificación (Fuera del objeto certData)
const verifyUrl = `https://fatlin.web.app/verify-pending.html?id=${certId}`;

// ✅ 3. Ahora construimos el objeto certData usando la variable previa
const certData = {
  ...
  // El QR ahora usa la variable verifyUrl ya procesada
  qr: `https://fatlin.web.app/verify-pending.html?id=${certId}`
...
document.getElementById('btn-dip-close').onclick = () => document.getElementById('modal-diploma').classList.remove('active');
